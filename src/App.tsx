...
function App() {
  ...
  const [notas, setNotas] = useState<{ 
    [key: string]: { 
      n1_b1: number; n2_b1: number; 
      n1_b2: number; n2_b2: number; 
      mediaB1: number; mediaB2: number; mediaFinal: number;
    } 
  }>({});
  ...
  const salvarNotasNoLocalStorage = (notasAtualizadas: typeof notas) => {
    if (semestreSelecionado !== null) {
      localStorage.setItem(`notas_${semestreSelecionado}`, JSON.stringify(notasAtualizadas));
    }
  };

  const handleNotaChange = (disciplina: string, campo: keyof typeof notas[string], valor: number) => {
    const notaCorrigida = Math.min(10, Math.max(0, Number(valor.toFixed(1))));

    setNotas((prev) => {
      const novaNota = { ...prev[disciplina], [campo]: notaCorrigida };
      novaNota.mediaB1 = Number(((novaNota.n1_b1 || 0) * 0.4 + (novaNota.n2_b1 || 0) * 0.6).toFixed(1));
      novaNota.mediaB2 = Number(((novaNota.n1_b2 || 0) * 0.2 + (novaNota.n2_b2 || 0) * 0.6 + (formativa || 0) * 0.2).toFixed(1));
      novaNota.mediaFinal = Number(((novaNota.mediaB1 + novaNota.mediaB2) / 2).toFixed(1));
      ...
      Object.keys(novasNotas).forEach((disciplina) => {
        novasNotas[disciplina].mediaB2 = Number(
          ((novasNotas[disciplina].n1_b2 || 0) * 0.2 + 
           ...
           notaCorrigida * 0.2).toFixed(1)
        );
        novasNotas[disciplina].mediaFinal = Number(((novasNotas[disciplina].mediaB1 + novasNotas[disciplina].mediaB2) / 2).toFixed(1));
      });
                ...
                <tr key={index}>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => handleNotaChange(disciplina.nome, "n1_b1", Number(e.target.value))}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      onChange={(e) => handleNotaChange(disciplina.nome, "n2_b1", Number(e.target.value))}
                    />
                  </td>
                  <td>{notas[disciplina.nome]?.mediaB1?.toFixed(1) || "-"}</td>
                  <td>
                    <input
                      type="number"
                      step="0.1"
                      ...
                      onChange={(e) => handleNotaChange(disciplina.nome, "n1_b2", Number(e.target.value))}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      onChange={(e) => handleNotaChange(disciplina.nome, "n2_b2", Number(e.target.value))}
                    />
                  </td>
                  <td>{notas[disciplina.nome]?.mediaB2?.toFixed(1) || "-"}</td>
                  <td>{notas[disciplina.nome]?.mediaFinal?.toFixed(1) || "-"}</td>
                </tr>
              ))
            )}
        </tbody>
